...
for numClusters in range(2,6):
    ...
    #styleIndex=[]
    accuracyText=[]
    numCorrectForThisStyle=[]
    allPossibleCorrectForThisStyle=[]
    for styleIndex in range(len(allPaintingStyles)):
        nameOfThisStyle=allPaintingStyles[styleIndex]
        howManyOfThisStyleThereAre=countOfEachPaintingStyle[nameOfThisStyle]
        howManyOfThisStyleAreInEachCluster=[]
        howManyFound=0
        for clusterIndex in range(numClusters):
            howManyOfThisStyleAreInEachCluster.append(0)
        #if allPaintingStyles[style]=='Tubism':
            #print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$tubism",howManyOfThisStyleAreInEachCluster,howManyOfThisStyleThereAre)
        for thisLabelIndex in range(len(labels2)): #for each label, if the corresponding data is this style, then add it
            if paintingImageLabels[thisLabelIndex]==nameOfThisStyle:
                #if paintingImageLabels[thisLabelIndex]=='Tubism':
                    #print("_____________________$$$$$$$$$$$$$$$$$$$$$$$$$$$$$tubism found",howManyOfThisStyleAreInEachCluster,howManyOfThisStyleThereAre)
                howManyOfThisStyleAreInEachCluster[labels2[thisLabelIndex]]+=1
                howManyFound+=1
        maxNumberOfThisStyleInAnyCluster=-1
        for clusterIndex in range(numClusters):
            if howManyOfThisStyleAreInEachCluster[clusterIndex]>maxNumberOfThisStyleInAnyCluster:
                maxNumberOfThisStyleInAnyCluster=howManyOfThisStyleAreInEachCluster[clusterIndex]
                if nameOfThisStyle=='Tubism':
                    print('tubism found',maxNumberOfThisStyleInAnyCluster,maxNumberOfThisStyleInAnyCluster)
        accuracyOfThisStyle=round(maxNumberOfThisStyleInAnyCluster/howManyOfThisStyleThereAre,4)
        print("----------------accuracy for this style:",nameOfThisStyle,accuracyOfThisStyle,"n=",
              howManyOfThisStyleThereAre,maxNumberOfThisStyleInAnyCluster,howManyFound)
        accuracyForEachStyle.append((accuracyOfThisStyle,nameOfThisStyle,howManyOfThisStyleThereAre,styleIndex))
        styleText.append(nameOfThisStyle)
        #styleIndex.append(style)
        accuracyText.append(accuracyOfThisStyle)
        numCorrectForThisStyle.append(maxNumberOfThisStyleInAnyCluster)
        allPossibleCorrectForThisStyle.append(howManyOfThisStyleThereAre)
    fontsize1=8
    
    sortedVals=sorted(accuracyForEachStyle, key=lambda x: x[0],reverse=True)
    
    gmmaccax.set_title("GMM ACCURACY="+str(round(sum(accuracyText)/len(accuracyText),4))+", # samples="+str(rangeOfImagesToProcess)+", # clusters="+str(numClusters))
    ...
    gmmaccfig = pylab.gcf()
    gmmaccfig.canvas.set_window_title("gBar="+",n"+str(rangeOfImagesToProcess)+",k"+str(numClusters))
    #gmmaccfig = plt.figure()
    #gmmaccax = gmmaccfig.add_subplot(111)
    ...
    plt.show()
    #plt.tight_layout()
    plt.savefig("_____figures/gmmBars_"+str(rangeOfImagesToProcess)+"_"+str(numClusters)+"_"+str(numOfImagesToShowOnGraph), dpi=300)
    #rangeOfImagesToProcess=1000
    #numOfImagesToShowOnGraph=75
    #numClusters=2
    ...
    gmmaccfig2 = plt.figure()
    gmmaccax2 = gmmaccfig2.add_subplot(111)
    ...
    #styleIndex=[]
    accuracyText2=[]
    numCorrectForThisStyle2=[]
    allPossibleCorrectForThisStyle2=[]
    for styleIndex in range(len(allPaintingStyles)):
        nameOfThisStyle=allPaintingStyles[styleIndex]
        howManyOfThisStyleThereAre=countOfEachPaintingStyle[nameOfThisStyle]
        howManyOfThisStyleAreInEachCluster=[]
        ...
        #if allPaintingStyles[style]=='Tubism':
            #print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$tubism",howManyOfThisStyleAreInEachCluster,howManyOfThisStyleThereAre)
        for thisLabelIndex in range(len(labels3)): #for each label, if the corresponding data is this style, then add it
            if paintingImageLabels[thisLabelIndex]==nameOfThisStyle:
                #if paintingImageLabels[thisLabelIndex]=='Tubism':
                    #print("_____________________$$$$$$$$$$$$$$$$$$$$$$$$$$$$$tubism found",howManyOfThisStyleAreInEachCluster,howManyOfThisStyleThereAre)
                howManyOfThisStyleAreInEachCluster[labels3[thisLabelIndex]]+=1
                howManyFound+=1
        maxNumberOfThisStyleInAnyCluster=-1
        for clusterIndex in range(numClusters):
            if howManyOfThisStyleAreInEachCluster[clusterIndex]>maxNumberOfThisStyleInAnyCluster:
        ...
        print("----------------accuracy for this style:",nameOfThisStyle,accuracyOfThisStyle,"n=",
              howManyOfThisStyleThereAre,maxNumberOfThisStyleInAnyCluster,howManyFound)
        accuracyForEachStyle2.append((accuracyOfThisStyle,nameOfThisStyle,howManyOfThisStyleThereAre,styleIndex))
        styleText2.append(nameOfThisStyle)
        #styleIndex.append(style)
        accuracyText2.append(accuracyOfThisStyle)
        numCorrectForThisStyle2.append(maxNumberOfThisStyleInAnyCluster)
        allPossibleCorrectForThisStyle2.append(howManyOfThisStyleThereAre)
    fontsize1=8
    sortedVals2=sorted(accuracyForEachStyle2, key=lambda x: x[0],reverse=True)
    
    gmmaccax2.set_title("KMEANS ACCURACY="+str(round(sum(accuracyText2)/len(accuracyText2),4))+", # samples="+str(rangeOfImagesToProcess)+", # clusters="+str(numClusters))
    
    #res_list = [x[0] for x in accuracyForEachStyle]
    #sortedVals=sorted()
    bars2=gmmaccax2.barh(np.arange(len(sortedVals2))*2,[x[0] for x in sortedVals2])
    print('k ((((((((((((((((((((((',sortedVals2)
    #gmmaccax.set_xticks([x[3] for x in sortedVals])
    #gmmaccax.set_xticklabels([x[1] for x in sortedVals])
    plt.yticks(np.arange(len(sortedVals2))*2, [x[1] for x in sortedVals2], fontsize=fontsize1)
    #gmmaccax.set_xticklabels([x[1] for x in sortedVals])
    def autolabel2(rects):
        """Attach a text label above each bar in *rects*, displaying its height."""
        for rect in range(len(rects)):
            height = rects[rect].get_width()
            gmmaccax2.annotate('{}'.format('n='+str(sortedVals2[rect][2])),
                        xy=(height,rects[rect].get_y() - rects[rect].get_height() / 2),
                        xytext=(10, 0),  # 3 points vertical offset
                        textcoords="offset points",
                        ha='left', va='bottom', fontsize=fontsize1)
    ...
    #gmmaccax.bar(styleIndex,accuracyText,tick_label=styleText)
    #gmmaccax.set_title("GMM ACCURACIES")
    gmmaccfig2.tight_layout()
    gmmaccfig2 = pylab.gcf()
    gmmaccfig2.canvas.set_window_title("kBar="+",n"+str(rangeOfImagesToProcess)+",k"+str(numClusters))
    plt.show()
    
    
    plt.savefig("_____figures/kmeansBars_"+str(rangeOfImagesToProcess)+"_"+str(numClusters)+"_"+str(numOfImagesToShowOnGraph), dpi=300)
    ...
    ax.set_title("GMM CLUSTERS. ACCURACY="+str(round(sum(accuracyText)/len(accuracyText),4))+", # samples="+str(rangeOfImagesToProcess)+", # clusters="+str(numClusters))
    ax.set_xlabel('avg grayscale')
    ax.set_ylabel('avg saturation')
    ax.set_zlabel('non-uniformity')
    #print(featureMatrix,'\n',featureMatrix[0],'\n',featureMatrix[1],'\n\n\n\n\n',featureMatrix[:, 0])
    #for cluster in range(numClusters):
        #print('cluster: ', cluster,np.where(labels == cluster))
        #print(paintingImageLabels[np.where(labels == cluster)])
    ...
    fig.tight_layout()
    fig = pylab.gcf()
    fig.canvas.set_window_title("gClus="+",n"+str(rangeOfImagesToProcess)+",k"+str(numClusters))
    plt.show()
    
    plt.savefig("_____figures/gmmClusters_"+str(rangeOfImagesToProcess)+"_"+str(numClusters)+"_"+str(numOfImagesToShowOnGraph), dpi=300)
    #plt.ioff()
    #plt.close()
    ...
    ax0.set_title("KMEANS CLUSTERS. ACCURACY="+str(round(sum(accuracyText2)/len(accuracyText2),4))+", # samples="+str(rangeOfImagesToProcess)+", # clusters="+str(numClusters))
    ax0.set_xlabel('avg grayscale')
    ax0.set_ylabel('avg saturation')
    ax0.set_zlabel('non-uniformity')
    #print(featureMatrix,'\n',featureMatrix[0],'\n',featureMatrix[1],'\n\n\n\n\n',featureMatrix[:, 0])
    #for cluster in range(numClusters):
        #print('cluster: ', cluster,np.where(labels == cluster))
        #print(paintingImageLabels[np.where(labels == cluster)])
    ...
    fig0.tight_layout()
    fig0 = pylab.gcf()
    fig0.canvas.set_window_title("kClus="+",n"+str(rangeOfImagesToProcess)+",k"+str(numClusters))
    plt.show()
    
    plt.savefig("_____figures/kmeansClusters_"+str(rangeOfImagesToProcess)+"_"+str(numClusters)+"_"+str(numOfImagesToShowOnGraph), dpi=300)